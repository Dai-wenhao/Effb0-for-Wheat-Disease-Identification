...
## 5. 实验结果
### 5.1 核心指标对比
 ENGS与主流深度学习模型在小麦叶片病害分类任务上的性能对比如下，模型在精度、计算效率上均表现更优，尤其对相似病害（条锈病/叶锈病）的区分能力显著提升：

| 模型                     | 分类准确率（Accuracy） | FLOPs | 参数量（M） |
| :----------------------- | ---------------------: | ----: | ----------: |
| EfficientNet-B0（基线）  |                93.30% |  3.36 |        0.38 |
| + SKBlock                |                97.21% | 32.18 |        2.42 |
| + GeM                    |                98.75% |  3.36 |        0.38 |
| + SKB + GeM（本文）      |                98.81% | 32.18 |        2.42 |
